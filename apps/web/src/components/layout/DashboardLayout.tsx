...
import { 
  ...
  PanelRightClose, PanelRightOpen, HardDrive, Check
} from 'lucide-react';
...
export const DashboardLayout: React.FC<DashboardLayoutProps> = ({
  ...
  const LABEL_COLORS = ['#3B82F6', '#10B981', '#8B5CF6', '#EF4444', '#F97316', '#F59E0B', '#14B8A6', '#EC4899'];

  const handleFormCreateLabel = async (e: React.FormEvent) => {
    e.preventDefault();
    ...
    >
      ...
      {isCreateLabelModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm animate-in fade-in">
          <div 
            style={{ backgroundColor: currentTheme.bgCard, borderColor: currentTheme.border }}
            className="w-full max-w-md rounded-2xl border shadow-2xl overflow-hidden p-6 space-y-4 animate-in zoom-in-95"
          >
            <div className="flex items-center justify-between">
              <h3 className="text-base font-bold text-white">New label</h3>
              <button 
                type="button" 
            ...
            <form onSubmit={handleFormCreateLabel} className="space-y-4">
              <div>
                ...
                <input
                  ...
                  style={{ backgroundColor: currentTheme.bgSidebar, borderColor: currentTheme.border }}
                  className="w-full border text-sm text-white rounded-xl px-3.5 py-2.5 outline-none focus:ring-2 focus:ring-blue-500 transition-all placeholder:text-slate-500"
                />
              </div>
                  ...
                  {LABEL_COLORS.map((c) => (
                    <button
                      ...
                      className={`w-7 h-7 rounded-full flex items-center justify-center transition-transform ${
                        newLabelColor === c ? 'scale-110 ring-2 ring-white ring-offset-2 ring-offset-black' : 'hover:scale-105'
                      }`}
                    >
                      {newLabelColor === c && <Check className="w-3.5 h-3.5 text-white stroke-[3]" />}
                    </button>
                  ))}
                </div>
              </div>
              ...
              <div className="flex items-center justify-end gap-2.5 pt-2">
                ...
                <button
                  ...
                  className="px-5 py-2 rounded-xl bg-blue-600 hover:bg-blue-500 disabled:opacity-50 text-xs font-semibold text-white transition-all shadow-md"
                >
