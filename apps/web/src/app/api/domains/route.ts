...
export async function GET(req: NextRequest) {
  try {
    ...
    const response = await fetch(`${API_BACKEND_URL}/v1/domains`, {
      method: 'GET',
      headers,
    });
    ...
    return NextResponse.json(
      { error: { message: error.message || 'Error fetching domains' } },
      { status: 502 }
    );
  }
}
...
export async function POST(req: NextRequest) {
  try {
    ...
    return NextResponse.json(
      { error: { message: error.message || 'Error creating domain' } },
      { status: 502 }
    );
  }
}
