...
function createDevToken(email: string = 'rahulkumar@eazzio.com'): string {
  ...
  const payload = Buffer.from(
    JSON.stringify({
      userId: 'b927c10d-19d6-4f75-9c4d-8e92f2f85a96',
      sessionId: 'sess_1',
      email,
...
export async function POST(req: NextRequest) {
  try {
    ...
    const response = await fetch(`${API_BACKEND_URL}/v1/messages/batch-action`, {
      method: 'POST',
      headers: {
    ...
    console.error('Batch action proxy error:', error);
    return NextResponse.json(
      { success: false, error: error.message || 'Batch action error' },
      { status: 502 }
    );
  }
}
